...
const ApplicationManagement = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.data.success) {
        setApplications(applications.filter((app) => (app._id || app.id) !== id));
        if (selectedApplication && (selectedApplication._id || selectedApplication.id) === id) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col md:flex-row bg-white rounded-lg shadow-sm overflow-hidden">
        {/* Applications List */}
        <div
          className={`w-full ${selectedApplication ? "hidden md:block md:w-2/5" : ""} overflow-y-auto`}
        >
        ...
        {selectedApplication && (
          <div className={`w-full ${selectedApplication ? "md:w-3/5" : ""} border-l border-gray-200 overflow-y-auto`}>
            {/* You can add your resume viewer or detailed view here */}
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-gray-600">{selectedApplication.jobSeekerInfo.email}</p>
                  <p className="text-gray-600">{selectedApplication.jobSeekerInfo.phone}</p>
                  <p className="text-gray-600">{selectedApplication.jobSeekerInfo.address}</p>
                  <div className="mt-2">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        selectedApplication.status === "pending"
                          ...
                          : selectedApplication.status === "final_rejected"
              ...
              {/* Other details like Applied Position, Applied Date, etc. */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="text-gray-600">{selectedApplication.jobInfo?.jobTitle}</p>
                </div>
                <div>
                  <h3 className="text-md font-semibold text-gray-700 mb-2 flex items-center">
                    <FaCalendarAlt className="mr-2" /> Applied Date
                  </h3>
                  <p className="text-gray-600">{selectedApplication.appliedDate}</p>
                </div>
              </div>

              {/* Candidate Rating and Cover Letter sections can be added here similarly */}
            </div>
          </div>
        )}
